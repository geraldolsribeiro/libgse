...
static int test_encap(int verbose, size_t frag_length, char* filename) {
  ...
  while ((packet = (unsigned char*)pcap_next(handle, &header)) != NULL) {
    ...
    for (i = 0; i < 6; i++)
      label[i] = i;
    status = gse_create_vfrag_with_data(&pdu[counter], in_size, GSE_MAX_HEADER_LENGTH, GSE_MAX_TRAILER_LENGTH, in_packet, in_size);
    if (status != GSE_STATUS_OK) {
  ...
  for (qos_idx = 0; qos_idx < QOS_NBR; qos_idx++) {
    ...
    do {
      ...
      do {
        status = gse_deencap_packet(vfrag_pkt[rcv_pkt_nbr], deencap, &label_type, rcv_label, &protocol, &rcv_pdu, &gse_length);
        rcv_pkt_nbr++;
      ...
      if (!compare_packets(verbose, rcv_pdu->start, rcv_pdu->length, cmp_packet + link_len_cmp, cmp_header.caplen - link_len_cmp)) {
