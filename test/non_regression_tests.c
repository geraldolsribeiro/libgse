...
static const size_t frag_length[20] = {
    128, 0, 1024, 256, 2048, 4096, 16, 64, 1024, 512, 256, 512, 4096, 64, 128, 1024, 2048, 512, 256, 1024,
};

static const size_t refrag_length[20] = {
    64, 1024, 512, 128, 32, 512, 16, 16, 256, 32, 128, 128, 2048, 16, 64, 512, 16, 128, 128, 64,
};
...
static int test_encap_deencap(int verbose, uint8_t label_type, int save, char* src_filename, char* frag_filename, char* refrag_filename);
static int open_pcap(char* filename, int verbose, pcap_t** handle, uint32_t* link_len);
static int get_gse_packets(int verbose, int save, gse_vfrag_t** vfrag_pkt, gse_encap_t** encap, pcap_t** frag_handle, pcap_dumper_t** frag_dumper, uint32_t link_len_frag,
                           uint32_t link_len_src, unsigned char* link_layer_head, int frag_length_idx, uint8_t qos, unsigned long pkt_nbr);
static int refrag(int verbose, int save, gse_vfrag_t** vfrag_pkt, gse_vfrag_t** refrag_pkt, pcap_t** refrag_handle, pcap_dumper_t** refrag_dumper, uint32_t link_len_refrag,
                  uint32_t link_len_src, unsigned char* link_layer_head, int refrag_length_idx, uint8_t qos, unsigned long pkt_nbr);
static int deencap_pkt(int verbose, gse_vfrag_t* vfrag_pkt, gse_vfrag_t* refrag_pkt, gse_deencap_t** deencap, pcap_t** cmp_handle, uint32_t link_len_cmp, unsigned long rcv_pkt_nbr,
                       unsigned long* rcv_tot_nbr, unsigned long pdu_counter);
static int compare_packets(int verbose, unsigned char* pkt1, int pkt1_size, unsigned char* pkt2, int pkt2_size);
...
int main(int argc, char* argv[]) {
  ...
  failure = test_encap_deencap(verbose, (label_type ? atoi(label_type) : 0), save, src_filename, frag_filename, refrag_filename);
...
static int test_encap_deencap(int verbose, uint8_t label_type, int save, char* src_filename, char* frag_filename, char* refrag_filename) {
  ...
  if (!save) {
    if (refrag_filename != NULL) {
      DEBUG(verbose,
            "Compare fragmented packets with those in %s\n"
  ...
  while ((packet = (unsigned char*)pcap_next(src_handle, &header)) != NULL) {
    ...
    /* Encapsulate the input packets, use in_packet and in_size as
       input */
    status = gse_create_vfrag_with_data(&pdu, in_size, GSE_MAX_HEADER_LENGTH, GSE_MAX_TRAILER_LENGTH, in_packet, in_size);
    if (status != GSE_STATUS_OK) {
    ...
    do {
      ...
      if (get_gse_packets(verbose, save, &vfrag_pkt, &encap, &frag_handle, &frag_dumper, link_len_frag, link_len_src, link_layer_head, frag_length_idx, qos, pkt_nbr) != 0) {
        goto release_lib;
      }
      pkt_nbr++;
      tot_nbr++;
      ...
      if (refrag_filename != NULL) {
        ...
        if (refrag(verbose, save, &vfrag_pkt, &refrag_pkt, &refrag_handle, &refrag_dumper, link_len_refrag, link_len_src, link_layer_head, refrag_length_idx, qos, pkt_nbr) > 0) {
          goto release_lib;
        }
        if (refrag_pkt != NULL) {
          tot_nbr++;
      ...
      ret = deencap_pkt(verbose, vfrag_pkt, refrag_pkt, &deencap, &cmp_handle, link_len_cmp, rcv_pkt_nbr, &rcv_tot_nbr, pdu_counter);
      rcv_pkt_nbr++;
      refrag_pkt = NULL;
      vfrag_pkt = NULL;
      if (ret > 0) {
...
static int open_pcap(char* filename, int verbose, pcap_t** handle, uint32_t* link_len) {
  ...
  *handle = pcap_open_offline(filename, errbuf);
  ...
  if (link_layer_type != DLT_EN10MB && link_layer_type != DLT_LINUX_SLL && link_layer_type != DLT_RAW) {
    DEBUG(verbose,
          "link layer type %d not supported in dump (supported = "
          "%d, %d, %d)\n",
...
static int get_gse_packets(int verbose, int save, gse_vfrag_t** vfrag_pkt, gse_encap_t** encap, pcap_t** frag_handle, pcap_dumper_t** frag_dumper, uint32_t link_len_frag,
  ...
  status = gse_encap_get_packet_copy(vfrag_pkt, *encap, frag_length[frag_length_idx], qos);
  ...
  if (status == GSE_STATUS_OK) {
    ...
    if (!save) {
      ...
      if (!compare_packets(verbose, gse_get_vfrag_start(*vfrag_pkt), gse_get_vfrag_length(*vfrag_pkt), frag_packet + link_len_frag, frag_header.caplen - link_len_frag)) {
    ...
    } else /* The save option is activated */
    {
      if (*frag_dumper != NULL) {
        ...
        memcpy(output_frag + link_len_src, gse_get_vfrag_start(*vfrag_pkt), gse_get_vfrag_length(*vfrag_pkt));
        if (link_len_src != 0) {
...
static int refrag(int verbose, int save, gse_vfrag_t** vfrag_pkt, gse_vfrag_t** refrag_pkt, pcap_t** refrag_handle, pcap_dumper_t** refrag_dumper, uint32_t link_len_refrag,
  ...
  /* Refragment the GSE packet */
  status = gse_refrag_packet(*vfrag_pkt, refrag_pkt, 0, 0, qos, refrag_length[refrag_length_idx]);
  ...
  if (!save) {
    ...
    if (!compare_packets(verbose, gse_get_vfrag_start(*vfrag_pkt), gse_get_vfrag_length(*vfrag_pkt), refrag_packet + link_len_refrag, refrag_header.caplen - link_len_refrag)) {
    ...
    if (*refrag_pkt != NULL) {
      ...
      if (!compare_packets(verbose, gse_get_vfrag_start(*refrag_pkt), gse_get_vfrag_length(*refrag_pkt), refrag_packet + link_len_refrag, refrag_header.caplen - link_len_refrag)) {
  ...
  } else /* The save option is activated */
  {
    if (*refrag_dumper != NULL) {
      ...
      header.len = link_len_src + gse_get_vfrag_length(*vfrag_pkt);
      header.caplen = header.len;
      memcpy(output_refrag_first + link_len_src, gse_get_vfrag_start(*vfrag_pkt), gse_get_vfrag_length(*vfrag_pkt));
      if (link_len_src != 0) {
        // Copy link layer header from source packet
      ...
      if (*refrag_pkt != NULL) {
        ...
        memcpy(output_refrag_second + link_len_src, gse_get_vfrag_start(*refrag_pkt), gse_get_vfrag_length(*refrag_pkt));
...
static int deencap_pkt(int verbose, gse_vfrag_t* vfrag_pkt, gse_vfrag_t* refrag_pkt, gse_deencap_t** deencap, pcap_t** cmp_handle, uint32_t link_len_cmp, unsigned long rcv_pkt_nbr,
  ...
  status = gse_deencap_packet(vfrag_pkt, *deencap, &label_type, rcv_label, &protocol, &rcv_pdu, &gse_length);
  ...
  *rcv_tot_nbr = *rcv_tot_nbr + 1;
  ...
  if ((refrag_pkt != NULL) && (status != GSE_STATUS_PDU_RECEIVED)) {
    status = gse_deencap_packet(refrag_pkt, *deencap, &label_type, rcv_label, &protocol, &rcv_pdu, &gse_length);
    if ((status != GSE_STATUS_OK) && (status != GSE_STATUS_PDU_RECEIVED)) {
  ...
  if (status == GSE_STATUS_PDU_RECEIVED) {
    ...
    if (!compare_packets(verbose, gse_get_vfrag_start(rcv_pdu), gse_get_vfrag_length(rcv_pdu), cmp_packet + link_len_cmp, cmp_header.caplen - link_len_cmp)) {
      DEBUG(verbose, "PDU #%lu: generated PDU is not as attended\n", pdu_counter);
      goto free_pdu;
    }

    DEBUG_L2(verbose, "Complete PDU #%lu:\nLabel Type: %d | Protocol: %#.4x | Label: %.2d", pdu_counter, label_type, protocol, rcv_label[0]);
...
static int compare_packets(int verbose, unsigned char* pkt1, int pkt1_size, unsigned char* pkt2, int pkt2_size) {
  ...
  if (pkt1_size != pkt2_size) {
    DEBUG(verbose,
          "packets have different sizes (%d != %d), compare only the %d "
          "first bytes\n",
