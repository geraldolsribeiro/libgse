  ...
  while ((packet = (unsigned char*)pcap_next(src_handle, &header)) != NULL) {
    ...
    */
    ...
    do {
      ...
      ret = get_gse_packets(verbose, save, &vfrag_pkt, &encap, &frag_handle, &frag_dumper, link_len_frag, link_len_src, link_layer_head, frag_length_idx, qos, pkt_nbr);
...
static int get_gse_packets(int verbose, int save, gse_vfrag_t** vfrag_pkt, gse_encap_t** encap, pcap_t** frag_handle, pcap_dumper_t** frag_dumper, uint32_t link_len_frag,
  ...
  status = gse_encap_get_packet_no_alloc(vfrag_pkt, *encap, frag_length[frag_length_idx], qos);
