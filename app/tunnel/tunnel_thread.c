...
void* get_packet_thread(void* argv) {
  ...
  while (alive) {
    ...
    do {
      if (!arg->copy) {
        ret = gse_encap_get_packet(&vfrag_pkt, arg->encap, rand() % 1500 + 1, arg->qos);
      } else {
        ret = gse_encap_get_packet_copy(&vfrag_pkt, arg->encap, rand() % 1500 + 1, arg->qos);
      }
    ...
    if ((ret > GSE_STATUS_OK) && (ret != GSE_STATUS_FIFO_EMPTY)) {
      ...
      if (arg->err->error == 1) /* uniform error model */
      {
        if (nb_bytes + gse_get_vfrag_length(vfrag_pkt) >= arg->err->bytes_without_error) {
          ...
          nb_bytes = gse_get_vfrag_length(vfrag_pkt) - (arg->err->bytes_without_error - nb_bytes);
        }
      ...
      } else if (arg->err->error == 2) /* non-uniform/burst error model */
      {
        ...
        if (is_state_drop && is_timeout(last, now, 2)) {
          fprintf(stderr,
                  "THREAD GET %u: go back to normal state (too much time between "
                  "packets #%u and #%u)\n",
      ...
      if (!to_drop) {
        ...
        ret = write_to_udp(arg->to, arg->raddr, arg->port, gse_get_vfrag_start(vfrag_pkt) - 2, gse_get_vfrag_length(vfrag_pkt) + 2);
        if (ret != GSE_STATUS_OK) {
      ...
      if ((arg->refrag) && (refrag_pkt != NULL)) {
        ...
        if (arg->err->error == 1) /* uniform error model */
        {
          if (nb_bytes + gse_get_vfrag_length(refrag_pkt) >= arg->err->bytes_without_error) {
            ...
            dropped++;
            fprintf(stderr, "THREAD GET %u: error inserted, GSE packet #%u dropped\n", arg->qos, local_seq);
            nb_bytes = gse_get_vfrag_length(refrag_pkt) - (arg->err->bytes_without_error - nb_bytes);
          }
        ...
        if (!to_drop) {
          ...
          ret = write_to_udp(arg->to, arg->raddr, arg->port, gse_get_vfrag_start(refrag_pkt) - 2, gse_get_vfrag_length(refrag_pkt) + 2);
          if (ret != GSE_STATUS_OK) {
            fprintf(stderr, "THREAD GET %u: write_to_udp failed\n", arg->qos);
            goto error;
          }
...
void* udp2tun_thread(void* argv) {
  ...
  while (alive) {
    ...
    ret = gse_deencap_packet(vfrag_pkt, arg->deencap, &label_type, label, &protocol, &pdu, &gse_length);
