 ...
 */
static gse_status_t gse_deencap_create_ctx(gse_vfrag_t* partial_pdu, gse_deencap_t* deencap, gse_header_t header, uint32_t crc);
 ...
 */
static gse_status_t gse_deencap_add_frag(gse_vfrag_t* data, gse_deencap_t* deencap, gse_header_t header);
 ...
 */
static gse_status_t gse_deencap_add_last_frag(gse_vfrag_t* data, gse_deencap_t* deencap, gse_header_t header);
 ...
 */
static size_t gse_deencap_compute_pdu_length(uint16_t total_length, gse_label_type_t label_type, size_t tot_ext_length);
 ...
 *  @return            The CRC32
 */
static uint32_t gse_deencap_compute_crc(unsigned char* data, size_t length, uint32_t crc_init);
...
gse_status_t gse_deencap_packet(gse_vfrag_t* data, gse_deencap_t* deencap, uint8_t* label_type, uint8_t label[6], uint16_t* protocol, gse_vfrag_t** pdu, uint16_t* packet_length) {
  ...
  if ((data == NULL) || (deencap == NULL) || (label_type == NULL) || (protocol == NULL) || (packet_length == NULL) || (pdu == NULL)) {
  ...
  *packet_length = (((uint16_t)header.gse_length_hi << 8) | header.gse_length_lo) + GSE_MANDATORY_FIELDS_LENGTH;
  ...
  if (payload_type == GSE_PDU_FIRST_FRAG) {
    ...
    field_length = GSE_TOTAL_LENGTH_LENGTH + GSE_PROTOCOL_TYPE_LENGTH + gse_get_label_length(header.lt);
    crc = gse_deencap_compute_crc(packet->start + head_offset, field_length, GSE_CRC_INIT);
  }
  ...
  case GSE_PDU_COMPLETE: {
    ...
    *protocol = ntohs(header.complete_s.protocol_type);
    ...
    if (gse_is_ext_hdr(*protocol)) {
      ...
      ret = deencap->read_header_ext(packet->start, &tot_ext_length, &protocol_type, extension_type, deencap->opaque);
      if (ret < 0) {
      ...
      *protocol = protocol_type;
      ...
      status = gse_check_header_extension_validity(packet->start, &tot_ext_length, extension_type, &protocol_type);
      if (status != GSE_STATUS_OK) {
        goto free_packet;
      }
    ...
    *label_type = header.lt;
    memcpy(label, header.complete_s.label.six_bytes_label, label_length);
    /* Check if label is not '00:00:00:00:00:00' */
    if (label_length == 6 && memcmp(label, "\x0\x0\x0\x0\x0\x0", 6) == 0) {
    ...
    *pdu = packet;
    ...
    status = gse_create_vfrag_with_data(pdu, packet->length, deencap->head_offset, deencap->trail_offset, packet->start, packet->length);
  ...
  case GSE_PDU_LAST_FRAG: {
    ...
    *protocol = ctx->protocol_type;
    ...
    /* Create the virtual buffer containing the PDU with appropriated offsets */
    status = gse_create_vfrag_with_data(pdu, ctx->partial_pdu->length, deencap->head_offset, deencap->trail_offset, ctx->partial_pdu->start, ctx->partial_pdu->length);
...
static gse_status_t gse_deencap_create_ctx(gse_vfrag_t* partial_pdu, gse_deencap_t* deencap, gse_header_t header, uint32_t crc) {
  ...
  /* check Protocol Type */
  if (gse_is_ext_hdr(ntohs(header.first_frag_s.protocol_type)) && deencap->read_header_ext == NULL) {
    status = GSE_STATUS_EXTENSION_NOT_SUPPORTED;
    goto free_partial_pdu;
  }
  ...
  pdu_length = gse_deencap_compute_pdu_length(ctx->total_length, header.lt, ctx->tot_ext_length);
  ...
  if ((partial_pdu->vbuf->length - partial_pdu_start_offset) < pdu_length) {
     ...
     * too small */
    status = gse_create_vfrag_with_data(&(ctx->partial_pdu), pdu_length, 0, GSE_MAX_TRAILER_LENGTH, partial_pdu->start, partial_pdu->length);
    if (status != GSE_STATUS_OK) {
      goto free_partial_pdu;
    }
  ...
  memcpy(&(ctx->label), &(header.first_frag_s.label), gse_get_label_length(header.lt));

  /* Check if label is not '00:00:00:00:00:00' */
  if (header.lt == GSE_LT_6_BYTES && memcmp(&(ctx->label), "\x0\x0\x0\x0\x0\x0", 6) == 0) {
    status = GSE_STATUS_INVALID_LABEL;
    goto free_vfrag;
  }
  ctx->bbframe_nbr = 0;
...
static gse_status_t gse_deencap_add_frag(gse_vfrag_t* partial_pdu, gse_deencap_t* deencap, gse_header_t header) {
  gse_status_t status = GSE_STATUS_OK;
  gse_deencap_ctx_t* ctx;
  ...
  /* Compute the data field part of the CRC32 and store it */
  ctx->crc = gse_deencap_compute_crc(partial_pdu->start, partial_pdu->length, ctx->crc);
...
static gse_status_t gse_deencap_add_last_frag(gse_vfrag_t* partial_pdu, gse_deencap_t* deencap, gse_header_t header) {
  gse_status_t status = GSE_STATUS_OK;
  gse_deencap_ctx_t* ctx;
  uint32_t rcv_crc;
  ...
  if (gse_is_ext_hdr(ctx->protocol_type)) {
    ...
    ret = deencap->read_header_ext(partial_pdu->start, &(ctx->tot_ext_length), &protocol_type, extension_type, deencap->opaque);
    if (ret < 0) {
      status = GSE_STATUS_EXTENSION_CB_FAILED;
      goto free_ctx;
    }
    ...
    /* check extensions validity */
    status = gse_check_header_extension_validity(partial_pdu->start, &ctx->tot_ext_length, extension_type, &protocol_type);
    if (status != GSE_STATUS_OK) {
      goto free_ctx;
    }
  ...
  if (gse_deencap_compute_pdu_length(ctx->total_length, ctx->label_type, ctx->tot_ext_length) != ctx->partial_pdu->length) {
...
static size_t gse_deencap_compute_pdu_length(uint16_t total_length, gse_label_type_t label_type, size_t tot_ext_length) {
  return total_length - (size_t)gse_get_label_length(label_type) - GSE_PROTOCOL_TYPE_LENGTH - tot_ext_length;
}
