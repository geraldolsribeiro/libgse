...
static int ext_cb(unsigned char* ext, size_t* length, uint16_t* protocol_type, uint16_t extension_type, void* opaque);
...
static int test_deencap(int verbose, char* src_filename, char* cmp_filename) {
  ...
  while ((packet = (unsigned char*)pcap_next(handle, &header)) != NULL) {
    ...
    /* Deecapsulate the input packets, use in_packet and in_size as
       input */
    status = gse_create_vfrag_with_data(&gse_packet, in_size, GSE_MAX_HEADER_LENGTH, GSE_MAX_TRAILER_LENGTH, in_packet, in_size);
    if (status != GSE_STATUS_OK) {
    ...
    status = gse_deencap_packet(gse_packet, deencap, &label_type, label, &protocol, &pdu, &gse_length);
    ...
    if (status == GSE_STATUS_PDU_RECEIVED) {
      ...
      if (!compare_packets(verbose, pdu->start, pdu->length, cmp_packet + link_len_cmp, cmp_header.caplen - link_len_cmp)) {
        DEBUG(verbose, "PDU #%lu: generated PDU is not as attended\n", counter);
        goto free_pdu;
      }
      DEBUG(verbose, "Complete PDU #%lu:\nLabel Type: %d | Protocol: %#.4x | Label: %.2d", counter, label_type, protocol, label[0]);
...
static int compare_packets(int verbose, unsigned char* pkt1, int pkt1_size, unsigned char* pkt2, int pkt2_size) {
  ...
  if (pkt1_size != pkt2_size) {
    DEBUG(verbose,
          "PDU have different sizes (%d != %d), compare only the %d "
          "first bytes\n",
...
static int ext_cb(unsigned char* ext, size_t* length, uint16_t* protocol_type, uint16_t extension_type, void* opaque) {
  ...
  *protocol_type = (current_type.null_1 & 0xF) << 12 | (current_type.null_2 & 0x08) << 8 | (current_type.h_len & 0x07) << 8 | (current_type.h_type & 0xFF);
  ...
  *length = current_length;
  ...
  if (ext_info->length1 != *length && ext_info->length2 != *length) {
    DEBUG(ext_info->verbose,
          "Extensions length are incorrect: "
