...
static int test_add_ext(int verbose, size_t frag_length, int ext_nbr, char* src_filename, char* cmp_filename);
static int compare_packets(int verbose, unsigned char* pkt1, int pkt1_size, unsigned char* pkt2, int pkt2_size);
static int ext_cb(unsigned char* ext, size_t* length, uint16_t* extension_type, uint16_t protocol_type, void* opaque);
...
int main(int argc, char* argv[]) {
  ...
  failure = test_add_ext(verbose, atoi(frag_length), atoi(ext_nbr), src_filename, cmp_filename);
...
static int test_add_ext(int verbose, size_t frag_length, int ext_nbr, char* src_filename, char* cmp_filename) {
  ...
  if (link_layer_type_src != DLT_EN10MB && link_layer_type_src != DLT_LINUX_SLL && link_layer_type_src != DLT_RAW) {
    DEBUG(verbose,
          "link layer type %d not supported in source dump (supported = "
          "%d, %d, %d)\n",
  ...
  if (link_layer_type_cmp != DLT_EN10MB && link_layer_type_cmp != DLT_LINUX_SLL && link_layer_type_cmp != DLT_RAW) {
    DEBUG(verbose,
          "link layer type %d not supported in comparison dump "
          "(supported = %d, %d, %d)\n",
          link_layer_type_cmp, DLT_EN10MB, DLT_LINUX_SLL, DLT_RAW);
  ...
  while ((packet = (unsigned char*)pcap_next(handle, &header)) != NULL) {
    ...
    status = gse_create_vfrag_with_data(&vfrag, in_size, GSE_MAX_HEADER_LENGTH, GSE_MAX_TRAILER_LENGTH, in_packet, in_size);
    ...
    if (update_crc) {
      ...
      if (status != GSE_STATUS_OK && status != GSE_STATUS_PARTIAL_CRC) {
      ...
      status = gse_encap_add_header_ext(vfrag, &vfrag_pkt, &tmp_crc, ext_cb, frag_length, 0, 0, qos, &opaque);
      if (status != GSE_STATUS_OK && status != GSE_STATUS_PARTIAL_CRC) {
    ...
    if (!compare_packets(verbose, vfrag->start, vfrag->length, cmp_packet + link_len_cmp, cmp_header.caplen - link_len_cmp)) {
    ...
    if (vfrag_pkt != NULL) {
      ...
      if (!compare_packets(verbose, vfrag_pkt->start, vfrag_pkt->length, cmp_packet + link_len_cmp, cmp_header.caplen - link_len_cmp)) {
        DEBUG(verbose, "packet #%lu: generated packet is not as attended\n", counter);
        goto free_packets;
      } else {
...
static int ext_cb(unsigned char* ext, size_t* length, uint16_t* extension_type, uint16_t protocol_type, void* opaque) {
  ...
  if (ext_info->length > *length) {
    DEBUG(ext_info->verbose,
          "Not enough space for extensions:\n"
