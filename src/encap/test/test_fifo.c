...
static int test_encap(int verbose, uint8_t qos_nbr, char* src_filename, char* cmp_filename);
...
static int test_encap(int verbose, uint8_t qos_nbr, char* src_filename, char* cmp_filename) {
  ...
  } while ((status != GSE_STATUS_FIFO_EMPTY) || (qos < qos_nbr));
