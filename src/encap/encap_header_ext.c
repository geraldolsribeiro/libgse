...
gse_status_t gse_encap_add_header_ext(gse_vfrag_t* packet, gse_vfrag_t** frag, uint32_t* crc, gse_encap_build_header_ext_cb_t callback, size_t max_packet_length,
                                      size_t head_offset, size_t trail_offset, uint8_t qos, void* opaque) {
...
restart:
  ...
  available_start_offset = (packet->start - packet->vbuf->start) - head_offset;
  ...
  if (payload_type == GSE_PDU_COMPLETE) {
    ...
    if (new_packet_length > max_packet_length) {
      status = gse_refrag_packet(packet, frag, head_offset, trail_offset, qos, max_packet_length - header_shift);
      if (status != GSE_STATUS_OK) {
        goto error;
      }
      goto restart;
     ...
     * else reallocate it */
    if (available_space < new_packet_length || available_start_offset < header_shift) {
      status = gse_reallocate_vfrag(packet, head_offset + header_shift, new_packet_length, head_offset, trail_offset);
      if (status != GSE_STATUS_OK) {
        goto error;
      }
    }
    ...
    new_header->gse_length_hi = ((gse_length + header_shift) >> 8) & 0x0F;
...
gse_status_t gse_encap_update_crc(gse_vfrag_t* packet, uint32_t* crc) {
  ...
  data = packet->start + GSE_MANDATORY_FIELDS_LENGTH + GSE_FRAG_ID_LENGTH;
  length = packet->length - GSE_MANDATORY_FIELDS_LENGTH - GSE_FRAG_ID_LENGTH - label_length;
  ...
  if (payload_type == GSE_PDU_LAST_FRAG) {
    ...
    memcpy(packet->end - GSE_MAX_TRAILER_LENGTH, &tmp_crc, GSE_MAX_TRAILER_LENGTH);
  } else {
    *crc = tmp_crc;
    status = GSE_STATUS_PARTIAL_CRC;
  }
