...
static int test_vfrag_robust(int verbose) {
  ...
  status = gse_create_vfrag_with_data(&vfrag, VFRAG_LENGTH, 0, 0, data, BAD_DATA_LENGTH);
  ...
  status = gse_create_vfrag_with_data(&vfrag, VFRAG_LENGTH, 0, 0, data, DATA_LENGTH);
  if (status != GSE_STATUS_OK) {
    DEBUG(verbose, "Error %#.4x when creating fragment (%s)\n", status, gse_get_status(status));
    goto failure;
  }
